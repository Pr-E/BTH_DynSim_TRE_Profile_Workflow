"""Discover table families, date fields and identifier candidates.

Discovery is intentionally heuristic and data-driven: candidate keys are
inferred from column names, uniqueness and observed cross-table overlap rather
than from hard-coded BTH schemas. Candidate roles still require TRE-side review.
"""
...
def infer_family(df):
    """Infer a broad table family from structural column-name evidence."""
    cols = {str(c).lower() for c in df.columns}

    # Family inference is deliberately broad: it routes EDA logic, not clinical semantics.
...
def identifier_candidates(df, min_score, max_candidates):
    """Rank plausible identifier columns without assuming a fixed source schema."""
    rows = []
...
def date_candidates(df, threshold):
    """Profile columns that meet the configured date-detection threshold."""
    rows = []
    parsed = {}
    for column in df.columns:
...
def resolve_cross_table_patient_keys(table_states):
    ...
    # Cache identifier value sets once so repeated pairwise overlap checks remain deterministic and efficient.
    value_sets = {}
    ...
    for name, state in table_states.items():
        ...
        for _, row in ids.iterrows():
            ...
            # Patient identifiers are often repeated across event rows; record IDs are usually near-unique.
...
def resolve_event_key(df, family, ids, patient_key):
    """Select a plausible record/event key after the patient-like key is resolved."""
    if ids.empty:
        return None
    ...
    # Event keys favour high-cardinality identifier-like fields after excluding the patient key.
...
def find_named_column(df, tokens):
    """Find the first column whose lower-case name contains every requested token."""
    for column in df.columns:
