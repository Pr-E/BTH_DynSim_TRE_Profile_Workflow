"""Orchestrate the complete TRE EDA and calibration workflow.

The runner performs source discovery, key resolution, table-specific EDA,
cross-table relationship QA and final aggregate-profile construction in a
...
def run_eda_calibration(input_dir, output_dir, config_path):
    """Run the complete TRE EDA/calibration pipeline and return in-memory states, results and aggregates."""
    ...
    for path in files:
        ...
        # `states` keeps source data and discovery decisions in memory; raw rows are not written to outputs.
        states[table] = {
            "path": path,
            "raw": df,
            "family": family,
    ...
    # The manifest documents provenance and process version without embedding patient-level content.
    manifest = {
        "profile_version": "dynsim_tre_eda_calibration_v1_3_1",
