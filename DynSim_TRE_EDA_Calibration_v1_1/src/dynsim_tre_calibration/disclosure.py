"""Prepare disclosure-controlled aggregate outputs for TRE egress review.

This module does not authorise egress. It applies the threshold and rounding
rules supplied at runtime, then rebuilds portable metadata only from the
disclosure-controlled copies.
"""
...
# Columns interpreted as counts for small-cell suppression and optional rounding.
COUNT_COLUMNS = {
    ...
    "duplicate_id_values_n", "rows_with_repeated_id_n", "max_rows_per_id",
}

# Statistics that may reveal a suppressed small count are blanked on the same unsafe row.
DERIVED_COLUMNS = {
...
def _numeric(s):
    """Coerce a series to numeric for threshold checks while leaving the source frame unchanged."""
    return pd.to_numeric(s, errors="coerce")


def _small(s, threshold):
    """Return cells with positive counts below the runtime disclosure threshold."""
    x = _numeric(s)
    return x.notna() & (x > 0) & (x < threshold)


def protect_frame(df, threshold, round_base):
    """Suppress small counts and dependent statistics, then apply optional aggregate count rounding."""
    out = df.copy()
    count_cols = [c for c in COUNT_COLUMNS if c in out.columns]
    unsafe = pd.Series(False, index=out.index)

    # Conservative row rule: if any supporting count is unsafe, suppress row-level derived statistics.
...
def prepare_egress(internal_dir, export_dir, threshold, round_base, bundle_path):
    """Create a disclosure-controlled export candidate and rebuild portable metadata from protected files only."""
    ...
    # Apply disclosure controls file-by-file; the internal profile itself remains untouched inside the TRE.
    ...
    # Rebuild portable metadata from protected CSVs so unsuppressed internal values cannot leak into the bundle.
    save_json({
