"""Assemble EDA results into CSV outputs and portable aggregate metadata.

This module is the boundary between exploratory calculations and the
machine-readable calibration contract consumed downstream by DynSim.
"""
...
# Stable output names form the interface used by validation, disclosure control and DynSim.
OUTPUTS = {
...
def _concat(results, key):
    """Concatenate one result family across all tables, returning an empty DataFrame when absent."""
    frames = [
...
def write_profile(states, results, cross_table, output_dir):
    """Write individual aggregate EDA outputs and compile the portable metadata contract."""
    ...
    # Keep each EDA domain in its own CSV so TRE reviewers can inspect findings independently.
    assembled = {
    ...
    # Internal CSVs contain aggregate findings only; source rows are never written here.
    ...
    # Consolidate the same aggregates into a machine-readable structure for downstream calibration.
    portable = {
